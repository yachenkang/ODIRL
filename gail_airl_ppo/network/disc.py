...
class AIRLDiscrim(nn.Module):
    ...
    def f(self, states, dones, next_states):
        ...
        # return (rs + self.gamma * (1 - dones) * next_vs - vs) * 10
...
class ODIRLDiscrim(AIRLDiscrim):
    ...
    def __init__(self, state_shape, gamma,
        ...
        super().__init__(state_shape, gamma, 
                         hidden_units_r, 
                         hidden_units_v, 
                         hidden_activation_r, 
                         hidden_activation_v)
    ...
    def forward(self, states, dones, log_pis, next_states, delta_r=0, alpha=0):
        # Discriminator's output is sigmoid(f - log_pi).
        return self.f(states, dones, next_states) + alpha * delta_r - log_pis

    def calculate_reward(self, states, dones, log_pis, next_states, delta_r=0, alpha=0):
        with torch.no_grad():
            logits = self.forward(states, dones, log_pis, next_states, delta_r, alpha=0)
            return -F.logsigmoid(-logits)